            ...
            $.post('request.php',param,function(res){
                ...
                if(res.status === 500){
                    ...
                    if(curPage === 'orderInfo'){ 
                        ...
                        } else if (res.type == 'pay.alipay.native') {
                            $('body').popUpWin({
                                content:function(){
                                    return '<img src="'+res.code_img_url+'" /><div class="msg_default_box"><i class="icon60_qr pngFix"></i><p>请使用支付宝扫描<br>二维码以完成支付</p></div>';
                                },
                                closeCallback:function(){
                                    self.popWin = undefined;
                                    self.opts.qrCodeClose = true;
