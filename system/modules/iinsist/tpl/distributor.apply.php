...
添加时间: <input type="text" name="posttime1" id="posttime1" class="input-text posttime" readonly="readonly" value="<?php echo $search['start_time'] ? : ''; ?>" /> -  
...
<div class="table-list lr10">
    <table width="100%" cellspacing="0">
        ...
        <tbody>
        <?php if ( is_array( $apply_list ) )  { ?>
        <?php foreach ( (array)$apply_list as $v ) { ?>
            <tr>
                <td> <?php echo $v['uid']; ?> </td>
                <td> <?php echo $v['username']; ?> </td>
                <td> <?php echo $status_arr[$v['apply_status']]; ?> </td>
                <td> <?php echo date( "Y-m-d H:i:s", $v['apply_time'] ); ?> </td>
                <?php if ( 0 == $v['apply_status'] ) { ?>
                <td class="action">
                <a href="<?php echo G_ADMIN_PATH; ?>/distributor/distributor_handle/1/<?php echo $v['apply_id']; ?>">
                通过
                </a>|
                <a href="<?php echo G_ADMIN_PATH; ?>/distributor/distributor_handle/2/<?php echo $v['apply_id']; ?>">
                拒绝
                </a>
                </td>
                <?php } ?>
