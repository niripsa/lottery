...
<div class="table-list lr10">
    <table width="100%" cellspacing="0">
        ...
        <tbody>
        <?php if ( is_array( $with_list ) )  { ?>
        <?php foreach ( (array)$with_list as $v ) { ?>
            <tr>
                <td> <?php echo $v['uid']; ?> </td>
                <td> <?php echo $v['username']; ?> </td>
                <td> <?php echo $v['with_money']; ?> </td>
                <td> <?php echo $v['card_num']; ?> </td>                
                <td> <?php echo $status_arr[$v['status']]; ?> </td>
                <td> <?php echo date( "Y-m-d H:i:s", $v['add_time'] ); ?> </td>
                <?php if ( 0 == $v['status'] ) { ?>
                <td class="action">
                <a href="<?php echo G_ADMIN_PATH; ?>/distributor/withdrawals_handle/<?php echo $v['with_id']; ?>">
                通过
                </a>
                </td>
                <?php } ?>
